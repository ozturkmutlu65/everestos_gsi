...
#
# Why is it required?
# This script gathers all the patchestd applied to make the GSI work,
# and then later re-apply the patchestd to other distros.
...
if git fetch --tags $original_remote || git fetch --tags $original_remote || git fetch --tags $original_remote || git fetch --tags $original_remote;then
	...
	patchestd_out=$TOP/patchestd/$compact_remote/
	mkdir -p $patchestd_out
	git format-patch $lastTag..HEAD -o $patchestd_out
fi
